...
classify_for <- function(beta, xtrain, ytrain, xtest, ytest){
  ...
  disc_loc  <- function(x, m, beta) as.numeric(crossprod(x - m, beta))
  
  for(i in 1:length(ytest)) ypred[i] <- ifelse(disc_loc(xtest[i, ], x1_bar, beta) < disc_loc(xtest[i, ], x2_bar, beta) , 1, 2)
